...
- `internal/ui/prompt_test.go` (新增) — 可选：huh 交互测试（若 huh 支持非交互模式）；否则归入手动验收
...
- stdout 不含 password 值（NFR-05/09）
...
**Depends on**：T1, T4（需要 NewProfileAppStore 已实现，且需要能 login 创建测试状态）
...
| US-07 | AC-07-3 | T2,T3,T4,T5,T6 | 每个命令实现自己的错误路径 + 下一步建议 |
...
| E2E-006 | T3 | E2E-018 | T6,T4 | E2E-030 | T2-T6 |

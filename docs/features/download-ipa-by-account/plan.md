...
```
T1 (app search) ──┐
                   ├──► T3 (download core) ──► T4 (download edge)
T2 (lib list+Store)┤
                   └──► T5 (library clean)
```
...
**并行轨道**：
- T1 + T2 可并行（互不依赖）
- T5 可在 T2 完成后与 T3 并行（互不依赖）

**依赖理由**：
- T3 依赖 T1：复用 resolveProfile helper + mockAppStore 扩展模式 + adapter AccountInfo 方法
- T3 依赖 T2：download 成功后调 `libraryStore.Add` 记录索引
- T4 依赖 T3：在 T3 的核心 download 编排上叠加 retry/edge case 逻辑
- T5 依赖 T2：clean 使用 library.Store 的 Remove/RemoveVersion/CleanAll 方法
...
| **E2E Cases** | E2E-001, E2E-002, E2E-002a, E2E-003, E2E-004, E2E-005, E2E-006, E2E-035, E2E-036, E2E-037 |
| **Files to create** | `internal/cli/app.go`（RENAME from apps.go）；`internal/cli/app_search_test.go`；`internal/appstore/query.go`（AppInfo / AccountInfoResult 类型 + 转换 helper） |
...
| **Required tests** | E2E-001~006/002a/035~037（CLI mock）；resolveProfile 单元测试（helpers_test.go） |
| **Acceptance command** | `go test ./internal/cli/ -run "TestAppSearch|TestResolveProfile" -count=1 && go build ./... && go vet ./...` |
...
| **Acceptance command** | `go test ./internal/library/ -count=1 && go test ./internal/cli/ -run TestLibraryList -count=1 && go build ./... && go vet ./...` |
| **Completion criteria** | Spock review pass + tests pass + build + vet |
...
| **Acceptance Criteria** | AC-02-1, AC-02-2, AC-02-3, AC-02-4, AC-02-9, AC-03-1, AC-03-2, AC-11-1, AC-11-2 |
| **E2E Cases** | E2E-007, E2E-008(manual), E2E-009, E2E-010, E2E-016, E2E-019, E2E-020(manual), E2E-037, E2E-046, E2E-047 |
...
| **Required tests** | download happy path + skip + not-found + multi-version + --profile；progress/validateOutputPath/isInteractive 单元 |
| **Acceptance command** | `go test ./internal/cli/ -run "TestDownload|TestValidateOutput|TestIsInteractive" -count=1 && go test ./internal/appstore/ -count=1 && go build ./... && go vet ./...` |
| **Completion criteria** | Spock review pass + tests pass + build + vet |
...
### T4: `app download` — retry + edge cases

| 维度 | 内容 |
|------|------|
| **User Stories** | US-06 (P2), US-07 (P2), US-09 (P3), US-10 (P3) |
| **Acceptance Criteria** | AC-02-5, AC-02-6, AC-02-7, AC-02-8, AC-02-10, AC-02-11, AC-09-1, AC-09-2, AC-10-1, AC-10-2, AC-10-3, AC-10-4, AC-10-5, AC-10-6 |
| **E2E Cases** | E2E-011, E2E-012, E2E-013, E2E-014, E2E-015, E2E-017, E2E-018, E2E-038, E2E-039, E2E-040, E2E-041, E2E-042, E2E-043, E2E-044, E2E-045 |
| **Files to modify** | `internal/cli/app_download.go`（+license retry / token retry / --force / --output validation / --external-version-id skip bypass / non-interactive）；`internal/appstore/adapter.go`（+Purchase/RefreshSession）；`internal/appstore/client_impl.go`（adapter 实现 Purchase/RefreshSession）；`internal/appstore/errors.go`（+ErrLicenseRequired/ErrPasswordTokenExpired 别名）；`internal/apperr/errors.go`（+ErrPaidAppNotSupported/ErrDownloadNonInteractive）；`internal/cli/auth_test.go`（mockAppStore +Purchase/RefreshSession 字段） |
| **Required tests** | license retry (yes/no/paid/non-interactive) + token retry + --force + --output validation (4 failures) + --external-version-id + --output tracked in list |
| **Acceptance command** | `go test ./internal/cli/ -run "TestDownloadEdge|TestDownloadLicense|TestDownloadToken|TestDownloadForce|TestDownloadOutput|TestDownloadVersion" -count=1 && go build ./... && go vet ./...` |
| **Completion criteria** | Spock review pass + tests pass + build + vet |
| **Rollback note** | 无新持久化（edge case 处理逻辑） |

**依赖**：T3（核心 download 编排）

### T5: `library clean` — 清理（全部/单 app/指定版本/非交互）
...
| **Acceptance command** | `go test ./internal/cli/ -run TestLibraryClean -count=1 && go build ./... && go vet ./...` |
| **Completion criteria** | Spock review pass + tests pass + build + vet |
...
|----|-----|------|
| US-01 | AC-01-1~6 | T1 |
| US-02 | AC-02-1~4, 02-9 | T3 |
| US-02 | AC-02-5~8, 02-10~11 | T4 |
| US-03 | AC-03-1~2 | T3 |
| US-04 | AC-04-1~3 | T2 |
| US-05 | AC-05-1~9 | T5 |
| US-05 | AC-05-10~12 | T5 |
| US-06 | AC-02-5~6 | T4 |
| US-07 | AC-02-7~8, 02-11 | T4 |
| US-08 | AC-08-1~3 | T1 (resolveProfile)；T3/T5 复用 |
| US-09 | AC-09-1~2 | T4 |
| US-10 | AC-10-1~6 | T4 |
| US-11 | AC-11-1~2 | T3 |
| US-11 | AC-05-10~12 | T5 |
...
| E2E | Task |
|-----|------|
| E2E-001~006, 002a, 035~037 | T1 |
| E2E-021~023 | T2 |
| E2E-007~010, 016, 019, 037, 046~047 | T3 |
| E2E-011~015, 017~018, 038~045 | T4 |
| E2E-024~034c, 048~050 | T5 |
| E2E-008, 020 (manual L4/L5) | T3 (validate 阶段手动) |
| E2E-N01~N05 (NFR) | 跨任务（NFR-04 全任务；NFR-05 T3；NFR-08 全任务；NFR-09 全任务；NFR-01 validate 手动） |

...
- **AC-05-8**：Given `library clean <bundle-id>` 但该 bundle-id 对应的物理文件已不存在（文件已被外部删除），When 运行，Then 显示 `"file already absent for '<bundle-id>'"`，exit 0（幂等）；再次运行 `library list` 时该 bundle-id 不再出现。
- **AC-05-9**：Given 非交互模式（stdin 非 TTY），When 运行 `library clean`（无论有无 bundle-id 参数），Then 不显示确认提示，显示 `"confirmation required in non-interactive mode; cannot proceed"` 错误，exit 1（安全默认：非交互不执行任何破坏性 clean 操作）。

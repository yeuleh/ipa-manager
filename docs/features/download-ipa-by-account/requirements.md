...
- `--output <path>` / `--external-version-id <id>` / `--force` / `--limit N` / `--purchase` flags。
...
- **AC-01-4**：Given `--limit N` flag，When 运行 `apps search <term> --limit N`，Then 结果数 ≤ N。
- **AC-01-5**：Given `--profile <id>` 指向有效且已登录的 profile，When 运行 `apps search <term> --profile <id>`，Then 使用该 profile 的 StoreFront（结果反映该账号的国家区域）。
...
- **AC-05-6**：Given clean 删除文件后，When 运行 `library list`，Then 被删除的条目不再出现（元数据索引与文件系统一致）。

### US-06 — Skip existing（幂等，已在 AC-02-5 / AC-02-6 覆盖）

> 无独立 AC；行为由 AC-02-5（跳过）与 AC-02-6（`--force` 覆盖）完整规定。

### US-07 — Free license prompt（已在 AC-02-7 / AC-02-8 覆盖）

> 无独立 AC；行为由 AC-02-7（免费授权提示）与 AC-02-8（付费拒绝）完整规定。
...
- **AC-09-1**：Given `--external-version-id <id>` flag 指向有效版本，When 运行 `install download <bundle-id> --external-version-id <id>`，Then 下载该指定版本（非最新），文件名版本段对应该版本，exit 0。
...
- **AC-10-2**：Given `--output <path>` 且该 profile 的 library 元数据索引，When 运行后 `library list`，Then 该 IPA 仍出现在列表中（元数据被记录，即使物理文件在自定义路径）。
...
- **`--purchase` flag**：本 mission 采用交互式提示（AC-02-7）而非显式 flag。若未来发现提示过于打扰，可在 design 或后续 mission 加 `--purchase` / `--no-purchase` flag 跳过提示。

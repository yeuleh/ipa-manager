...
- **AC-02-1**：Given 存在已登录的 active profile 且 bundle-id 有效，When 运行 `ipa-manager app download <bundle-id>`，Then IPA 下载到默认 library 路径（`<configRoot>/library/<profileID>/` 下），显示含最终路径的成功消息，exit 0。

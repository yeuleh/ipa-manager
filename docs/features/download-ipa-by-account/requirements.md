...
但**下载 IPA 文件的核心功能尚未实现**——`appstore.Search` / `appstore.Download` 返回 `ErrNotImplemented`，`library.IPAStore` 全是 stub，CLI 的 `apps search` / `download` 只打印 "not yet implemented"。
...
2. `ipa-manager download <bundle-id>` → IPA 下载到该账号的隔离 library
...
| Actor | Description |
|-------|-------------|
| ipa-manager user | 运行 `apps search` / `download` / `library list` / `library clean`，管理多账号下的 IPA |
...
- CLI 命令填实：
  ...
  - `download <bundle-id>` —— 下载 IPA 到 per-account library（顶层命令，非 install 子命令）
...
- **AC-02-3**：Given 无 active profile 或未登录，When 运行 `download`，Then 显示错误（指向 login/use），exit 1。
- **AC-02-4**：Given bundle-id 在 App Store 不存在，When 运行 `download <bundle-id>`，Then 显示 `"app not found: <bundle-id>"` 错误，exit 1。
- **AC-02-5**：Given 目标路径已存在同版本 IPA 且未传 `--force`，When 运行 `download <bundle-id>`，Then 显示 `"already exists: <path> (use --force to overwrite)"`，**不重新下载**，exit 0（幂等）。
- **AC-02-6**：Given 目标已存在且传 `--force`，When 运行 `download <bundle-id> --force`，Then IPA 被重新下载并覆盖原文件，exit 0。
- **AC-02-7**：Given 下载过程返回 `ErrLicenseRequired` 且 app 价格 = 0，When 运行 `download`，Then 提示 `"this app requires a free license. acquire? [Y/n]"`；选 yes → 获取授权后继续下载，exit 0；选 no → 取消，exit 0（非错误）。
- **AC-02-8**：Given 下载返回 `ErrLicenseRequired` 且 app 价格 > 0，When 运行 `download`，Then 显示 `"paid apps are not supported"` 错误，exit 1。
- **AC-02-9**：Given `--profile <id>` flag，When 运行 `download <bundle-id> --profile <id>`，Then 下载使用该 profile 的账号会话，IPA 存入该 profile 的 library 目录。
- **AC-02-10**：Given active profile 的 password token 已过期，When 运行 `download`，Then 工具使用 keychain 中存储的凭据透明重登录并重试下载；成功则下载完成 exit 0，重登录失败则显示错误 exit 1。
- **AC-02-11**：Given 非交互模式（stdin 非 TTY）且下载遇到 `ErrLicenseRequired`（免费 app），When 运行 `download`，Then 不显示提示（无法交互），显示 `"license acquisition requires interactive confirmation; cannot proceed non-interactively"` 错误，exit 1。
...
- **AC-08-2**：Given `--profile <id>` 指向已存在但未登录的 profile，When 运行 `apps search` 或 `download` 带 `--profile <id>`，Then 显示 `"profile '<id>' has no credentials"`，exit 1。
...
- **AC-09-1**：Given `--external-version-id <id>` flag 指向有效版本，When 运行 `download <bundle-id> --external-version-id <id>`，Then 下载该指定版本（非最新），成功消息显示该版本号，且 `library list` 中该条目的 Version 列对应该指定版本，exit 0。
...
- **AC-10-1**：Given `--output <path>` flag，When 运行 `download <bundle-id> --output <path>`，Then IPA 保存到 `<path>`（而非默认 library 路径），exit 0。
...
- **命令结构决议**（用户确认）：`download` 提升为**顶层命令** `ipa-manager download <bundle-id>`，而非 `install` 子命令。理由：下载 ≠ 安装（下载只拉文件到本地，不涉及设备操作）；download 是本工具最高频核心操作，顶层位置最短最好打（类比 `git pull` / `docker pull`）。`install` 组净化为纯设备操作（`push` / `uninstall` / `update`）。library 管理为独立的本地资源管理命令组。重构后命令树：
  ```
  download <bundle-id>               ← 顶层（核心操作）
  apps search / versions             ← App Store 查询
  install push / uninstall / update  ← 纯设备操作
  library list / clean               ← 本地文件管理
  ```

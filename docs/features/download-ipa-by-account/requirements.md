...
但**下载 IPA 文件的核心功能尚未实现**——`appstore.Search` / `appstore.Download` 返回 `ErrNotImplemented`，`library.IPAStore` 全是 stub，CLI 的 `apps search` / `install download` 只打印 "not yet implemented"。
...
1. `ipa-manager apps search <名字>` → 找到目标 app 的 bundle-id
2. `ipa-manager install download <bundle-id>` → IPA 下载到该账号的隔离 library
...
| Actor | Description |
|-------|-------------|
| ipa-manager user | 运行 `apps search` / `install download` / `library list` / `library clean`，管理多账号下的 IPA |
...
- CLI 命令填实：
  - `apps search <term>` —— 按名字模糊搜索，表格输出
  - `install download <bundle-id>` —— 下载 IPA 到 per-account library
...
- `apps versions <bundle-id>`（列举历史版本）—— 未来 mission。
...
- **AC-01-1**：Given 存在已登录的 active profile，When 运行 `ipa-manager apps search <term>`，Then 以表格输出搜索结果（列：Name / Bundle-ID / Version / Price），exit 0。
- **AC-01-2**：Given 无 active profile 或 active profile 未登录，When 运行 `apps search`，Then 显示错误提示（指向 `auth login` 或 `accounts use`），exit 1。
- **AC-01-3**：Given 搜索返回零结果，When 运行 `apps search <term>`，Then 显示 `"no results found for '<term>'"`，exit 0。
- **AC-01-4**：Given `--limit N` flag（N 为正整数），When 运行 `apps search <term> --limit N`，Then 结果数 ≤ N。
- **AC-01-6**：Given `--limit` 值为 0、负数或非整数，When 运行 `apps search <term> --limit <invalid>`，Then 显示 `"invalid --limit value: must be a positive integer"` 错误，exit 1。
- **AC-01-5**：Given `--profile <id>` 指向有效且已登录的 profile，When 运行 `apps search <term> --profile <id>`，Then 使用该 profile 的 StoreFront（结果反映该账号的国家区域）。验证方法：用两个不同区域的 profile 搜索同一 term，观察结果集/可用性差异。

### US-02 — Download（下载 IPA）

- **AC-02-1**：Given 存在已登录的 active profile 且 bundle-id 有效，When 运行 `ipa-manager install download <bundle-id>`，Then IPA 下载到默认 library 路径（`<configRoot>/library/<profileID>/` 下），显示含最终路径的成功消息，exit 0。
- **AC-02-2**：Given 下载成功，When 检查 library 目录，Then 对应 IPA 文件存在且大小 > 0。
- **AC-02-3**：Given 无 active profile 或未登录，When 运行 `install download`，Then 显示错误（指向 login/use），exit 1。
- **AC-02-4**：Given bundle-id 在 App Store 不存在，When 运行 `install download <bundle-id>`，Then 显示 `"app not found: <bundle-id>"` 错误，exit 1。
- **AC-02-5**：Given 目标路径已存在同版本 IPA 且未传 `--force`，When 运行 `install download <bundle-id>`，Then 显示 `"already exists: <path> (use --force to overwrite)"`，**不重新下载**，exit 0（幂等）。
- **AC-02-6**：Given 目标已存在且传 `--force`，When 运行 `install download <bundle-id> --force`，Then IPA 被重新下载并覆盖原文件，exit 0。
- **AC-02-7**：Given 下载过程返回 `ErrLicenseRequired` 且 app 价格 = 0，When 运行 `install download`，Then 提示 `"this app requires a free license. acquire? [Y/n]"`；选 yes → 获取授权后继续下载，exit 0；选 no → 取消，exit 0（非错误）。
- **AC-02-8**：Given 下载返回 `ErrLicenseRequired` 且 app 价格 > 0，When 运行 `install download`，Then 显示 `"paid apps are not supported"` 错误，exit 1。
- **AC-02-11**：Given 非交互模式（stdin 非 TTY）且下载遇到 `ErrLicenseRequired`（免费 app），When 运行 `install download`，Then 不显示提示（无法交互），显示 `"license acquisition requires interactive confirmation; cannot proceed non-interactively"` 错误，exit 1。
- **AC-02-9**：Given `--profile <id>` flag，When 运行 `install download <bundle-id> --profile <id>`，Then 下载使用该 profile 的账号会话，IPA 存入该 profile 的 library 目录。
- **AC-02-10**：Given active profile 的 password token 已过期，When 运行 `install download`，Then 工具使用 keychain 中存储的凭据透明重登录并重试下载；成功则下载完成 exit 0，重登录失败则显示错误 exit 1。
...
- **AC-04-1**：Given active profile 有已下载的 IPA，When 运行 `ipa-manager library list`，Then 以表格输出（列：Bundle-ID / Version / Size / Downloaded-At），exit 0。
...
- **AC-05-1**：Given active profile 的 library 有 N > 0 个 IPA，When 运行 `library clean`（无参数），Then 显示确认提示 `"remove all N IPA(s) for profile '<id>'? [y/N]"`；选 yes → 全部删除 + 成功消息（含删除数量）；选 no → `"cancelled"`，exit 0。
...
- **AC-05-6**：Given clean 删除文件后，When 运行 `library list`，Then 被删除的 bundle-id 不再出现在列表中。
- **AC-05-7**：Given 某 IPA 是通过 `--output <path>` 下载到自定义路径（非默认 library 目录）且仍被 library 索引跟踪，When 运行 `library clean <bundle-id>`，Then 确认提示显示该 IPA 的**完整自定义路径**（如 `"remove '<bundle-id>' at '/Users/x/custom/app.ipa'? [y/N]"`）；选 yes → 删除该路径下的文件 + 移除索引条目；选 no → cancelled，exit 0。
- **AC-05-8**：Given `library clean <bundle-id>` 但该 bundle-id 对应的物理文件已不存在（仅索引残留），When 运行，Then 移除索引条目并显示 `"file already absent, removed index entry for '<bundle-id>'"`，exit 0（幂等）。
- **AC-05-9**：Given 非交互模式（stdin 非 TTY），When 运行 `library clean`（无参数），Then 不显示确认提示，显示 `"confirmation required in non-interactive mode; cannot proceed"` 错误，exit 1（安全默认：非交互不执行破坏性操作）。
...
- **AC-08-2**：Given `--profile <id>` 指向已存在但未登录的 profile，When 运行 `apps search` 或 `install download` 带 `--profile <id>`，Then 显示 `"profile '<id>' has no credentials"`，exit 1。
...
- **AC-09-1**：Given `--external-version-id <id>` flag 指向有效版本，When 运行 `install download <bundle-id> --external-version-id <id>`，Then 下载该指定版本（非最新），成功消息显示该版本号，且 `library list` 中该条目的 Version 列对应该指定版本，exit 0。
...
- **AC-10-1**：Given `--output <path>` flag，When 运行 `install download <bundle-id> --output <path>`，Then IPA 保存到 `<path>`（而非默认 library 路径），exit 0。
...
| NFR-10 | Performance — search | 搜索响应以 Apple iTunes API 为上界（通常 < 3s）；本工具额外开销 < 200ms（profile 解析 + keychain 读）。 | 手动计时：`apps search` 端到端 < 5s（正常网络）。 |
...
- **`install download` vs 新命令组**：download 留在现有 `install` 命令组下（它是 install 流程的前置步骤）。library 管理独立为新 `library` 命令组（概念上是本地资源管理，非设备操作）。

...
| MF-02      | T2     | E2E-002 / E2E-003 通过 `err.Error()` 间接断言 stderr 内容(因为 cobra root 把 RunE 返回的 err 渲染为 `Error: <err.Error()>`),而非通过 `execute(..., &out, &errOut)` 显式捕获 stderr buffer | 接受 residual risk。DD-09(root.go 错误渲染契约)已被 `device_test.go:1020-1030` 单独覆盖,且本 mission 的 stderr 子串断言(`STDQ`/`password token is expired`)对 cobra 的"Error:" 前缀无依赖。validate 阶段如要加固可改用 `execute()` 显式捕获。 |

...
| ⚠️ keyring 包 | ipatool **v2.3.0 tag** 使用 `github.com/99designs/keyring v1.2.1`（与 tech-stack.md 一致；初稿误记 byteness/keyring，Stage 4 评审已更正）。wrapper 包裹 ipatool 的 `pkg/keychain.Keychain` 接口，与底层实现无关。 |
...
### 1.4 keyring 包（已由 Stage 4 评审修正）

> ⚠️ 本节初稿曾误记为 `byteness/keyring`（基于 master HEAD），Stage 4 评审针对 **v2.3.0 release tag** 重新核验源码后更正如下：

**ipatool v2.3.0** 锁定版本实际使用 `github.com/99designs/keyring v1.2.1`（与 tech-stack.md 原描述一致）。`pkg/keychain/keyring.go` 直接 `import "github.com/99designs/keyring"`，`cmd/common.go` 用 `keyring.Open(keyring.Config{...})`。

实现以锁定的 `v2.3.0` tag 的 go.mod 为准：`github.com/99designs/keyring v1.2.1`。ProfileKeychain wrapper 包裹的是 ipatool 的 `pkg/keychain.Keychain` 接口，与底层 keyring 实现无关。

来源：`majd/ipatool` **v2.3.0 tag** 的 `go.mod`、`pkg/keychain/keyring.go`、`cmd/common.go`（GitHub raw，Stage 4 评审 2026-06-28 复核）
